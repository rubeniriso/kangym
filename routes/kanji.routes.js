...
router.get("/:userId", isAuthenticated, async (req, res, next) => {
  ...
  if (user === undefined) {
    res.json({ errorMessage: "User undefined" });
    return;
  }
  try {
    const kanjis = await UserKanjiModel.find(
      { user: user },
      function (err, user) {
          
      }
    ).where("scheduledReview").lte(Date().toLocaleDateString());
...
router.patch("/:username", isAuthenticated, async (req, res, next) => {
  ...
  try {
    ...
    if (error.codeName === "DuplicateKey") {
      res.json({
        errorMessage: "We cant update user, username already in use.",
      });
      return;
    }
    next(error);
...
router.patch("/:id/edit", async (req, res, next) => {
  const { id } = req.params;
  const { password, repeatPassword, username, location, description } =
    req.body;
  // Checking all inputs
  ...
  if (password && repeatPassword) {
    if (password.length < 8 || repeatPassword < 8) {
      res
        .status(400)
        .json({
          errorMessage: "Passwords need to have more than 8 characters.",
        });
      return;
    }
    // Passwords match
    ...
    if (passwordRegex.test(password) === false) {
      res
        ...
        .json({
          errorMessage:
            "Invalid password, needs 8 characters, one letter and one number.",
        });
      return;
    }
  }
